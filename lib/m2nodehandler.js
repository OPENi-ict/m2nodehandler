...
function sender(config){
   ...
   if (config.isMongrel2){
      ...
      return {
         'send' : function (uuid, connId, responseObj){
            console.log("MONGREL2 - INTERNAL")
            //console.log(JSON.stringify({'uuid':uuid, 'connId':connId, 'res':responseObj}))
            return send.send( JSON.stringify({'uuid':uuid, 'connId':connId, 'res':responseObj}) )
         }
      }
   }
   else {
